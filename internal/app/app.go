	...
	circuitBreaker := gobreaker.NewCircuitBreaker(
		gobreaker.Settings{
			...
			OnStateChange: func(name string, from gobreaker.State, to gobreaker.State) {
				log.Printf("Circuit Breaker: %s from %s to %s\n", name, from, to)
			},
		},
	)
...
func (a *App) runGRPCServer() error {
	log.Printf("Starting gRPC server... on address: [%v]", a.serviceProvider.GRPCConfig().Address())
...
func (a *App) runHTTPServer() error {
	log.Printf("Starting http server... on address: [%v]", a.serviceProvider.HTTPConfig().Address())
...
func (a *App) runSwaggerServer() error {
	log.Printf("Starting swagger server... on address: [%v]", a.serviceProvider.SwaggerConfig().Address())
...
func (a *App) runPrometheusServer() error {
	log.Printf("Starting prometheus server... on address: [%v]", a.serviceProvider.PrometheusConfig().Address())
...
func serveSwaggerFile(path string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("Serve swagger file: [%s]", path)
		...
		log.Printf("swagger file served: [%s]", path)
	}
}
